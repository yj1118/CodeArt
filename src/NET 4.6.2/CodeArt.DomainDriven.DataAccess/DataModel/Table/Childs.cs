    ...
    public partial class DataTable
    {
        ...
        private void InitBuildtimeChilds()
        {
            ...
            foreach (var field in this.ObjectFields)
            {
                ...
                if (type.IsList()) type = type.ResolveElementType();
                if (field.FieldType == DataFieldType.GeneratedField
                    || field.FieldType == DataFieldType.Value
                    || field.FieldType == DataFieldType.ValueList) continue;
        ...
        private DataTable CreateChildTable(DataTable master, IDataField memberField, Type objectType)
        {
            ...
            switch (memberField.FieldType)
            {
                ...
                case DataFieldType.EntityObjectPro:
                    {
                        table = CreateEntityObjectProTable(root, master, memberField, objectType);
                        break;
                    }
                case DataFieldType.EntityObjectProList:
                    {
                        table = CreateEntityObjectProListTable(root, master, memberField, objectType);
                        break;
                    }
        ...
        private static DataTable CreateEntityObjectProTable(DataTable root, DataTable master, IDataField memberField, Type objectType)
        {
            ...
            return DataTable.Create(root,
                                ...
                                objectType,
                                master.IsSnapshot,//根据从表，判断是否为快照
                                DataTableType.EntityObjectPro, fields, memberField);
        }
        ...
        private static DataTable CreateEntityObjectProListTable(DataTable root, DataTable master, IDataField memberField, Type objectType)
        {
            //需要创建EntityObjectPro从表和中间表
            var slave = CreateEntityObjectProTable(root, master, memberField, objectType);
            var middle = CreateMiddleTable(slave, memberField);
            slave.Middle = middle;
            return slave;
        }
            ...
            _getRuntimeTable = LazyIndexer.Init<DataTable, Func<string, Func<Type, DataTable>>>((master) =>
                            {
                                return LazyIndexer.Init<string, Func<Type, DataTable>>((propertyName) =>
                                {
                                    ...
                                    {
                                        Type objectType = propertyType.IsList() ? propertyType.ResolveElementType() : propertyType;
