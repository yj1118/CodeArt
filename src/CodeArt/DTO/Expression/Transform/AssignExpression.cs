...
namespace CodeArt.DTO
{
    ...
    internal class AssignExpression : TransformExpression
    {
        ...
        private static Func<string, (bool StartRoot, string FindExp)> _getValueExpression = LazyIndexer.Init<string, (bool StartRoot, string ValueFindExp)>((valueFindExp) =>
         {
             bool startRoot = valueFindExp.StartsWith("@");
             if (startRoot) valueFindExp = valueFindExp.Substring(1);
             return (startRoot, valueFindExp);
         });
