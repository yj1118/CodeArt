...
namespace CodeArt.Web.WebPages.Xaml.Script
{
    ...
    public class ScriptView : IScriptView
    {
        ...
        /// 返回上一页
        /// </summary>
        /// <param name="reload">是否刷新</param>
        public void Back(bool reload = false)
        {
            _scriptBody.Append("history.go(-1);");
            if(reload)
            {
                _scriptBody.Append("location.reload();");
            }
        }

        public void Reload()
        {
            _scriptBody.Append("location.reload(true);");
        }
