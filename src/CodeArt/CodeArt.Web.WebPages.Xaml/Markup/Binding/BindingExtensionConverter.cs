...
{
    [SafeAccess]
    public class BindingExtensionConverter : TypeConverter
    {
        ...
        protected override object ConvertTo(string value)
        {
            ...
            if (string.IsNullOrEmpty(expression)) throw new XamlException("绑定表达式" + value + "格式不正确");
            return new BindingExtension(expression);
        }
