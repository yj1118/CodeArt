...
namespace CodeArt.Web.WebPages.Xaml
{
    ...
    public class DependencyProperty : IDependencyProperty
    {
        ...
        public static void Output(string fileName)
        {
            ...
            foreach (var dp in _properties)
            {
                code.AppendFormat("{0} {1}", dp.OwnerType.Name, dp.Name);
                code.AppendLine();
            }
            code.AppendFormat("registered:{0}", _registered.Count);
            code.AppendLine();
            foreach (var p in _registered)
            {
                code.AppendFormat("{0}", p.Key.Name);
                code.AppendLine();
            }
        ...
        /// <summary>
        /// 尝试执行静态构造函数，如果已执行了的不再执行
        /// </summary>
        /// <param name="objectType"></param>
        /// <returns></returns>
        private static void TryStaticConstruct(Type dType)
        {
            var obj = Activator.CreateInstance(dType) as DependencyObject;
            if (obj.ObjectType != dType)
            {
                throw new ApplicationException("未知的错误");
            }
            DependencyProperty.MarkRegistered(dType);
        }
