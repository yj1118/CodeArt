$$.createModule("metronic.tinymce", function (api, module) {
    ...
    var $tinymce = $$.metronic.tinymce = function () {
        ...
        function init(o) {
            var core = getCore(o);
            var id = "tinymceTextarea_" + util.getId();
            core.attr("id", id);

            tinymce.init({
                selector: 'textarea#' + id,
                language: "zh_CN",
                plugins: 'print preview fullscreen code powerpaste searchreplace autolink directionality advcode visualblocks visualchars fullscreen link mediaembed codesample table charmap hr pagebreak nonbreaking anchor toc insertdatetime advlist lists wordcount tinymcespellchecker a11ychecker imagetools textpattern formatpainter permanentpen pageembed mentions linkchecker imagePro',
                menubar: false,
                toolbar: 'insertfile undo redo | styleselect | bold italic strikethrough forecolor backcolor emoticons | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | imagePro table link | removeformat | print preview fullscreen code wordcount',
                height: parseInt(o.attr("data-height")),
    ...
    tinymce.PluginManager.add('imagePro', function (editor, url) {

        var context = new function () {
            this.resize = function () { //将图片的原始大小设置到input里去
                var file = this.file;
                if (file) {
                    var form = $$("#tinymceImageForm");
                    form.set({
                        width: file.width,
                        height: file.height
                    });
                    this._scale = { width: file.width, height: file.height, };
                }
            }
            this.clear = function () {
                this.file = null;
            }
            this.scale = function () { //获得图片设置的比例
                if (!this.file) return null;
                var s = this._scale;
                return {
                    width: s.width,
                    height: s.height,
                    getWidth: function (h) { //给高度，根据比例计算宽度
                        return h ? parseInt(this.width / this.height * h) : 0;
                    },
                    getHeight: function (w) { //给宽度，根据比例计算高度
                        return w ? parseInt(this.height / this.width * w) : 0;
                    },
                }
                //var s = {
                //    width: $$("#tinymceImageDialog .width").get(),
                //    height: $$("#tinymceImageDialog .height").get()
                //};
                //if (!s.width || !s.height) s = { width: this.file.width, height: this.file.height };
                //return s;
            }
        }
        ...
        function getMetadata(image) {
            ...
            var g = $(image);
            var src = g.attr("src");
            var width = src.match(/w=(\d+)/);
            if (!width) return null;
            width = width[1];

            var height = src.match(/h=(\d+)/);
            if (!height) return null;
            height = height[1];

            var quality = src.match(/q=(\d+)/);
            if (!quality) return null;
            quality = quality[1];

            var key = src.match(/key=(.*)/);
            if (!key) return null;
            key = key[1];

            var displayWidth = g.attr("width");
            var displayHeight = g.attr("height");

            return {
                displayWidth: displayWidth ? (displayWidth == '100%' ? 0 : displayWidth) : 0,
                displayHeight: displayHeight ? (displayHeight == '100%' ? 0 : displayHeight) : 0,
                width: width,
                height: height,
                quality: quality,
                key: key
        ...
        function init() {
            var form = $$("#tinymceImageForm");
            var dropzone = $$("#tinymceImageForm [data-dropzone='true']");
            dropzone.uploaded = function (file) {
                context.file = file;
                context.resize();
            }

            var image = getSelectedImage(editor);
            var m = getMetadata(image);
            if (m) {
                form.accept({
                    image: {
                        StoreKey: m.key
        ...
        editor.ui.registry.addButton('imagePro', {
            icon: "image",
            tooltip: '插入/编辑图片',
            onAction: function () {
                init();

                var dialog = $$("#tinymceImageDialog");
                var dropzone = $$("#tinymceImageForm [data-dropzone='true']");
                var baseUrl = dropzone.attr("data-loadthumbnailurl");
                dialog.open();

                var ok = $("#tinymceImageDialog .tinymceOK");
                ok.off("click");
                ok.on("click", function () {
                    ...
                    var g = data.image;
                    if (!g) {
                        $$metronic.sweetAlert.error("请上传图片");
                        return;
                    }
                    ...
                    var width = data.width ? data.width : 1600;
                    var height = data.height ? data.height : 800;
                    var quality = data.quality ? data.quality : 30;
                    p.src = baseUrl + "?w=" + width + "&h=" + height + "&c=2&q=" + quality + "&key=" + (g.StoreKey || g.storeKey);
                    p.width = data.displayWidth ? data.displayWidth : "100%";
                    p.height = data.displayHeight ? data.displayHeight : ""; //高度自动
                    insertOrUpdateImage(editor, p);
                    dialog.close();
                });

                var displayWidth = $$("#tinymceImageDialog .displayWidth");
                displayWidth.changed = function () {
                    set(this, displayHeight, "height");
                }

                var displayHeight = $$("#tinymceImageDialog .displayHeight");
                displayHeight.changed = function () {
                    set(this, displayWidth, "width");
                }

                var width = $$("#tinymceImageDialog .width");
                width.changed = function () {
                    set(this, height, "height");
                }

                var height = $$("#tinymceImageDialog .height");
                height.changed = function () {
                    set(this, width, "width");
                }

                function set(o, obj, type) {
                    if (context.ignoreScale) return;

                    var f = context.scale();
                    if (!f) return;

                    context.ignoreScale = true;

                    var value = type == "width" ? f.getWidth(o.get()) : f.getHeight(o.get());
                    obj.set(value);

                    context.ignoreScale = false;
                }
            }
        });
    ...
    var create = function (normalizeCss, data) {
        ...
        setAttrib(image, 'data-upload', 'true');
        if (data.caption) {
    ...
    var insertOrUpdateImage = function (editor, data) {
        ...
        if (image) {
            if (data.src) {
                ...
                //writeImageDataToSelection(editor, data); 这是原插件的，不适合我们的需要
            } else {
                deleteImage(editor, image);
            }
        } else if (data.src) {
