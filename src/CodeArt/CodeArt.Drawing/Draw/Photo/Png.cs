...
{
    public class Png:IPhotoHandler
    {
        private Png() { }
        ...
        public void ThumbByFill(Stream sourceStream, Stream ouputStream, int width, int height, bool highQuality,Color background)
        {
            Image source = null;
            Graphics gdiobj = null;
            try
            {
                ...
                if (width == 0 || (width > 500 && width > source.Width)) width = source.Width;//做500的限制,实际文件超过200的，就以实际为大小
                if (height == 0 || (height > 500 && height > source.Height)) height = source.Height;//做500的限制,实际文件超过200的，就以实际为大小
                ...
                using (Bitmap img = new Bitmap(width, height))
                {
                    ...
                    gdiobj.InterpolationMode = InterpolationMode.HighQualityBicubic;
                    gdiobj.PixelOffsetMode = PixelOffsetMode.HighQuality;
                    gdiobj.FillRectangle(new SolidBrush(Color.FromArgb(0, background)), 0, 0, width, height);
