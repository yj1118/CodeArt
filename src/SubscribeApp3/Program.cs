...
namespace SubscribeApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            EventPortal.Subscribe("Event2", new Handler1());
            EventPortal.Subscribe("Event2", new Handler2());

            Console.WriteLine("group1 已订阅事件Event1和Event2，按任意键退出程序......");
            Console.ReadLine();

            EventPortal.Cancel("Event1");
            EventPortal.Cancel("Event2");
        }

        [SafeAccess]
        private class Handler1 : EventHandler
        {
            protected override void DynamicHandle(string eventName, dynamic @event)
            {
