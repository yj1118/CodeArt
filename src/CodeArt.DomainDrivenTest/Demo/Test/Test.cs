...
namespace CodeArt.DomainDrivenTest.Demo
{
    ...
    public class Test : DomainStage
    {
        ...
        {
            {
                var books = DataPortal.Query<Book>("mainBookmark.id in (1,2,3)[key aa]", (arg) =>
                {
                }, QueryLevel.None);
        ...
        public static void AssertBook(Book book)
        {
            ...
            DataPortal.Direct<Book>((conn) =>
            {
                ...
                var bookmarksData = conn.Query("select * from Book_Bookmarks where BookId=@BookId", new { BookId = book.Id });
            ...
            DataPortal.Direct<Book>((conn) =>
            {
                var datas = conn.Query("select * from Book_Posters where BookId=@BookId", new { BookId = book.Id });
                AssertBookPosters(book, datas, book.Posters, conn);
            });

            DataPortal.Direct<Book>((conn) =>
            {
                var datas = conn.Query("select * from Book_RelatedCategories where BookId=@BookId", new { BookId = book.Id });
            ...
            DataPortal.Direct<Book>((conn) =>
            {
                ...
                var datas = conn.Query("select * from Book_SaleAddresses where BookId=@BookId", new { BookId = book.Id });
        ...
        {
            var data = conn.QueryFirstOrDefault("select * from BookCover where BookId=@BookId and id=@id", new { BookId = book.Id, Id = cover.Id });
            if (cover.IsEmpty())
            {
                Assert.IsNull(data);
                return;
            ...
            //查询中间表
            var authorsData = conn.Query("select * from BookCover_Authors where BookId=@BookId and BookCoverId=@BookCoverId", new { BookId = book.Id, BookCoverId = cover.Id });
            Assert.AreEqual(authorsData.Count(), cover.Authors.Count());
            foreach (dynamic temp in authorsData)
            {
                var authorId = temp.AuthorId;
        ...
        private static void AssertBookCovers(Book book, IEnumerable<dynamic> datas, IEnumerable<BookCover> covers, IDbConnection conn)
        {
            ...
            {
                var bookCoverId = temp.BookCoverId;
        ...
        {
            var data = conn.QuerySingle("select * from dbo.Author where  BookId=@BookId and id=@id", new { BookId = book.Id, Id = obj.Id });
        ...
        {
            var data = conn.QuerySingle("select * from Bookmark where BookId=@BookId and id=@id", new { BookId = book.Id, Id = obj.Id });
            ...
            //查询中间表
            var readersData = conn.Query("select * from Bookmark_Readers where BookId=@BookId and BookmarkId=@BookmarkId", new { BookId = book.Id, BookmarkId = obj.Id });
            Assert.AreEqual(readersData.Count(), obj.Readers.Count());
            foreach (dynamic temp in readersData)
            {
                var bookReaderId = temp.BookReaderId;
            ...
            //查询中间表
            var coversData = conn.Query("select * from Bookmark_Covers where BookId=@BookId and BookmarkId=@BookmarkId", new { BookId = book.Id, BookmarkId = obj.Id });
        ...
        private static void AssertBookmarks(Book book, IEnumerable<dynamic> datas, IEnumerable<Bookmark> bookmarks, IDbConnection conn)
        {
            ...
            foreach (dynamic temp in datas)
            {
                var bookmarkId = temp.BookmarkId;
        ...
        {
            var data = conn.QueryFirstOrDefault("select * from BookReader where  BookId=@BookId and id=@id", new { BookId = book.Id, Id = obj.Id });
            if (obj.IsEmpty())
            {
                Assert.IsNull(data);
                return;
        ...
        private static void AssertBookPosters(Book book, IEnumerable<dynamic> datas, IEnumerable<BookPoster> posters, IDbConnection conn)
        {
            ...
            foreach (dynamic temp in datas)
            {
                var bookPosterId = temp.BookPosterId;
        ...
        {
            var data = conn.QueryFirstOrDefault("select * from BookPoster where BookId=@BookId and id=@id", new { BookId = book.Id, Id = poster.Id });
            if (poster.IsEmpty())
            {
                Assert.IsNull(data);
                return;
        ...
        private static void AssertBookCategories(IEnumerable<dynamic> datas, IEnumerable<BookCategory> objs, IDbConnection conn)
        {
            ...
            foreach (dynamic temp in datas)
            {
                var bookCategoryId = temp.BookCategoryId;
        ...
        {
            var data = conn.QueryFirstOrDefault("select * from BookAddress where  BookId=@BookId and id=@id", new { BookId = book.Id, Id = obj.Id });
            if (obj.IsEmpty())
            {
                Assert.IsNull(data);
                return;
            ...
            var categoryDatas = conn.Query("select * from BookAddress_Categories where BookId=@BookId and BookAddressId=@BookAddressId", new { BookId = book.Id, BookAddressId = obj.Id });
            AssertBookCategories(categoryDatas, obj.Categories, conn);

            AssertBookCover(book, obj.Photo, conn);
            var photoDatas = conn.Query("select * from BookAddress_Photos where BookId=@BookId and BookAddressId=@BookAddressId", new { BookId = book.Id, BookAddressId = obj.Id });
        ...
        private static void AssertBookAddresses(Book book, IEnumerable<dynamic> datas, IEnumerable<BookAddress> addresses, IDbConnection conn)
        {
            ...
            foreach (dynamic temp in datas)
            {
                var bookAddressId = temp.BookAddressId;
        ...
        public static int GetAssociatedCount<T>(Book book, object id)
        {
            ...
            string sql = string.Format("select AssociatedCount from {0} where bookId=@bookId and id=@id", tableName);
            int count = 0;
            DataPortal.Direct<Book>((conn) =>
            {
                var result = conn.ExecuteScalar(sql, new { BookId = book.Id, Id = id });
